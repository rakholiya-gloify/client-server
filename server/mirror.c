...
// Function prototypes
void handle_mirror_request(int client_socket, int mirror_socket);
...
int main()
{
    ...
    if (mirror_socket == -1)
    {
        DECLARE_STRING_TEMPLATE(er_socker_crear, "OOPS!! We Are Facing Error while creating mirror socket!!!");
        generic_printf(er_socker_crear);
        exit(EXIT_FAILURE);
    }

    // Set up mirror server address
    struct sockaddr_in mirror_address;
    mirror_address.sin_family = AF_INET;
    mirror_address.sin_addr.s_addr = INADDR_ANY;
    mirror_address.sin_port = htons(8081);

    // Bind mirror socket
    if (bind(mirror_socket, (struct sockaddr *)&mirror_address, sizeof(mirror_address)) == -1)
    {
        DECLARE_STRING_TEMPLATE(er_socker_bind, "OOPS!! We Are Facing Error while binding mirror socket!!!");
        generic_printf(er_socker_bind);
        close(mirror_socket);
        exit(EXIT_FAILURE);
    }
    if (listen(mirror_socket, 100) == -1)
    {
        DECLARE_STRING_TEMPLATE(er_mirror_con, "OOPS!! We Are Facing Error while listening for mirror connections!!!");
        generic_printf(er_mirror_con);
        close(mirror_socket);
        exit(EXIT_FAILURE);
    }

    DECLARE_STRING_TEMPLATE(mir_conn_msg, "Congratulations!! Mirror Server is up on 8081 port...!!!\n");
    generic_printf(mir_conn_msg);

    while (!terminate_flag)
    {
        int client_socket = accept(mirror_socket, NULL, NULL);
        if (client_socket == -1)
        {
            DECLARE_STRING_TEMPLATE(mirror_con, "OOPS!! We Are Facing Error while accepting mirror connection!!!");
        generic_printf(mirror_con);
            continue;
        }
        ...
        if (pid == -1)
        {
            DECLARE_STRING_TEMPLATE(fork_er, "OOPS!! We Are Facing Error while forking!!!");
        generic_printf(fork_er);
            close(client_socket);
            continue;
        }
        else if (pid == 0)
        {
            close(mirror_socket);
            handle_mirror_request(client_socket, mirror_socket);
            close(client_socket);
            exit(EXIT_SUCCESS);
        }
        else
...
void handle_mirror_request(int client_socket, int mirror_socket)
{
    ...
    while (!terminate_flag)
    {
        ...
        {
            terminte_client_request_cmd(mirror_socket, client_socket);
        }
        else
        {
        ...
        while ((entry = readdir(dir)) != NULL)
        {
            if (entry->d_type == DT_REG)
            {
                ...
                if (stat(filepath, &file_stat) == 0)
                {
                    ...
                    printf("Mirror File Path: %s\n", filepath);
                    printf("Mirror File Creation Time: %s", ctime(&file_time));
                    printf("Mirror Specified Time: %s", ctime(&specified_time));
                    printf("Mirror Time Difference: %f seconds\n", difftime(file_time, specified_time));
...
void terminte_client_request_cmd(int mirror_socket, int client_socket)
{
